...
class COCODetection(DatasetSplit):
    ...
    def eval_inference_results(self, results, output=None):
        print("coco.py eval_inference_results")
        continuous_id_to_COCO_id = {v: k for k, v in self.COCO_id_to_category_id.items()}
        for res in results:
            # convert to COCO's incontinuous category id
            # print(f"res : {res}")
            # res : {'image_id': 581781, 'category_id': 1, 'bbox': [502.5419, 115.2525, 515.3842, 146.3433], 'score': 0.0837}
            if res['category_id'] in continuous_id_to_COCO_id:
                # print(f"res[category_id] : {res['category_id']}")
                # print(f"changed res[category_id]: {continuous_id_to_COCO_id[res['category_id']]}")
